...
mod tests {
    ...
    fn test_corner_to_corner() {
        ...
        assert_eq!(nav.current(), Point { x: 31, y: 0 });
        assert!(steps <= 62);
    }

    #[test]
    fn test_maze_with_gaps() {
        let mut nav = Navigator::new();
        let mut g = [[false; 32]; 32];
        for x in 0..30 { g[x][5] = true; }
        g[30][5] = false;
        for x in 1..31 { g[x][10] = true; }
        g[0][10] = false;
        nav.set_grid(&g);
        assert!(nav.set_destination(15, 20));
        let mut steps = 0;
        loop {
            let r = nav.step();
            if r == 0 { break; }
    ...
    #[test]
    fn test_multi_waypoint_zigzag() {
        let mut nav = Navigator::new();
        nav.add_waypoint(10, 0);
        nav.add_waypoint(10, 10);
        nav.add_waypoint(0, 10);
        assert!(nav.set_destination(0, 0));
        loop {
            let r = nav.step();
            if r == -1 { panic!("blocked"); }
    ...
    #[test]
    fn test_mid_navigation_redirect() {
        let mut nav = Navigator::new();
        nav.set_destination(10, 0);
        nav.step(); nav.step();
        assert!(nav.set_destination(0, 5));
        loop {
            let r = nav.step();
            if r == 0 { break; }
            assert_eq!(r, 1);
        }
        assert_eq!(nav.current(), Point { x: 0, y: 5 });
    }

    #[test]
    fn test_single_row_corridor() {
        let mut nav = Navigator::new();
        let mut g = [[false; 32]; 32];
        for x in 1..32 { for y in 1..32 { g[x][y] = true; } }
        nav.set_grid(&g);
        assert!(nav.set_destination(20, 0));
        loop {
            let r = nav.step();
            if r == 0 { break; }
            assert_eq!(r, 1);
        }
        assert_eq!(nav.current(), Point { x: 20, y: 0 });
    }
    ...
    fn test_boundary_edges() {
        ...
        loop { if nav.step() == 0 { break; } }
        assert_eq!(nav.current(), Point { x: 0, y: 31 });
        assert!(nav.set_destination(31, 0));
        loop { if nav.step() == 0 { break; } }
        assert_eq!(nav.current(), Point { x: 31, y: 0 });
    }
    ...
    #[test]
    fn test_dynamic_obstacle_replan() {
        let mut nav = Navigator::new();
        assert!(nav.set_destination(5, 0));
        nav.step(); nav.step();
        let mut g = [[false; 32]; 32];
        g[3][0] = true; g[3][1] = true; g[3][2] = true;
        nav.set_grid(&g);
        let r = nav.step();
        assert!(r >= 0);
    }
    ...
    #[test]
    fn test_consecutive_destinations() {
        let mut nav = Navigator::new();
        nav.set_destination(5, 5);
        while nav.step() == 1 {}
        assert!(nav.at_destination());
        nav.set_destination(10, 0);
        while nav.step() == 1 {}
        assert!(nav.at_destination());
        nav.set_destination(0, 10);
        while nav.step() == 1 {}
        assert_eq!(nav.current(), Point { x: 0, y: 10 });
    }

    #[test]
    fn test_surrounded_origin() {
        let mut nav = Navigator::new();
        let mut g = [[false; 32]; 32];
        g[1][0] = true; g[0][1] = true;
        nav.set_grid(&g);
        assert!(nav.set_destination(0, 0));
        assert!(nav.at_destination());
    }
    ...
    #[test]
    fn test_snake_waypoints() {
        let mut nav = Navigator::new();
        nav.add_waypoint(5, 0);
        nav.add_waypoint(5, 5);
        nav.add_waypoint(0, 5);
        nav.add_waypoint(0, 10);
        assert!(nav.set_destination(5, 10));
        loop {
            let r = nav.step();
            if r == -1 { panic!("blocked"); }
            if r == 0 && nav.at_destination() { break; }
    ...
    #[test]
    fn test_blocked_dest_keeps_blocked_flag() {
        let mut nav = Navigator::new();
        nav.set_destination(5, 0);
        let mut g = [[false; 32]; 32];
        g[5][0] = true;
        nav.set_grid(&g);
        assert!(!nav.set_destination(5, 0));
    ...
    #[test]
    fn test_waypoint_clear_resets() {
        let mut nav = Navigator::new();
        assert_eq!(nav.next_waypoint(), nav.dest);
        nav.add_waypoint(3, 3);
        assert_eq!(nav.next_waypoint(), Point { x: 3, y: 3 });
        nav.clear_waypoints();
        assert_eq!(nav.next_waypoint(), nav.dest);
    }
}
